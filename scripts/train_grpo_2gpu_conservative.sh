...
# Moderate batch size (4 instead of 8)
ARGS+=" data.train_batch_size=4"
...
# Actor training - moderate mini batch size
...
# vLLM rollout - conservative settings
...
ARGS+=" actor_rollout_ref.rollout.response_length=96"
ARGS+=" actor_rollout_ref.rollout.dtype=float16"

# Training config - 2 epochs with checkpoints
ARGS+=" trainer.critic_warmup=0"
ARGS+=" trainer.total_epochs=2"
ARGS+=" trainer.test_freq=1"
ARGS+=" trainer.save_freq=500"
ARGS+=" trainer.output_dir='./outputs/grpo_kk_conservative'"
ARGS+=" trainer.logger=['console']"
ARGS+=" trainer.project_name='grpo_kk_conservative'"
ARGS+=" trainer.experiment_name='qwen2_1.5b_v100_2gpu_conservative'"
